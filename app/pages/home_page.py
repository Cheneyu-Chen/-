...
class HomePage(QWidget):
    def __init__(self) -> None:
        ...
        metrics_layout.addWidget(two_col_row(
            ...
            make_metric("成果输出", "图像 + 数据 + 分析报告", "支持导出图像，内置仿真成果对照、相似度分析和报告生成工具。"),
        ))
        ...
        modules_layout.addWidget(muted_label(
            ...
            "仿真成果对照：展示真实实验与仿真结果的对照、误差分析和大学物理课程联系。\n"
            "分析与报告：提供照片相似度、多边形有限差分、二维超材料阵列和自动报告生成。"
        ))
        layout.addWidget(modules)

        report, report_layout = make_card("成果结构")
        report_layout.addWidget(muted_label(
            "平台将物理原理、数值模型、动态仿真、教学案例、仿真成果对照和图像分析连接为完整流程，"
            "可直接服务课堂演示、实验讨论和作品汇报。"
        ))
